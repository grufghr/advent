...
def part01(input_data):
    ...
    for c in range(c_size):
        col = list(diag_report_np[:, c].astype(int))
...
def part02(input_data):
    ...
    for c in range(c_size):
        if len(ox_filter_list) > 1:
            ox_counts = np.bincount(list(np.array(ox_filter_list)[:, c].astype(int)))
        ...
        if len(co_filter_list) > 1:
            co_counts = np.bincount(list(np.array(co_filter_list)[:, c].astype(int)))
