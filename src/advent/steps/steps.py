...
def given_aoc(context):
    ...
    assert len(regex_result) != 1, f'Could not parse year/day from filename: {context.feature.filename}'
...
def given_input_file(context, input_file):
    ...
    context.input_data = context.puzzle.load_data(input_file)
...
def then_validate_feature(context):
    ...
    assert regex_result, f'Feature name pattern mismatch: {context.feature.name}'
