...
# -*- coding: utf-8 -*-
"""
Advent of Code - Solve Puzzle 2022 Day 07: No Space Left On Device
"""
import os
import re
...
def solve(terminal_output):
    ...
    # part 01
    part01_folder_sum = sum([f.size()
                            for f in folder_list if f.size() <= 100000])
    ...
    folder_list_space = [(f, f.size())
                         for f in folder_list if f.size() >= space_required]
