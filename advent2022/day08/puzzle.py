...
def solve(tree_map_np):
    ...
    for r in range(1, num_r - 1):
        for c in range(1, num_c - 1):
            ...
            visible_n, score_n = calc_score(outlook_n, height)

            outlook_e = tree_map_np[r, :][c + 1:num_c]
            visible_e, score_e = calc_score(outlook_e, height)

            outlook_s = tree_map_np[:, c][r + 1:num_r]
            visible_s, score_s = calc_score(outlook_s, height)

            outlook_w = np.flip(tree_map_np[r, :][0:c])
            visible_w, score_w = calc_score(outlook_w, height)
